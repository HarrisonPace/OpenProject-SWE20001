...
function validate() {
	...
	var email = /^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/;    /* regular expression for email id with proper syntax */
	var decimal =  /^(?=.*\d)(?=.*[a-z])(?=.*[A-Z])(?=.*[^a-zA-Z0-9])(?!.*\s).{8,15}$/;  /* regular expression for password with 8-15 letters 
	and one lowercase,uppercase,special character each. */
...
/* Rule 2, check if the email adress is properly written with full syntax such as thindamrit17@yahoo.com */
       if (! sid.match (email)) {
		   errMsg += "<li> Write full email syntax with .com or others.</li>";
	   }
...
/* Rule 4, check if password contains  letters and other chracters between 8 to 15 characters which contain at least one lowercase letter, 
...
if (! (pwd1 == pwd2) ) { //check whether Password is Re-entered
     errMsg += "<li>Password Does Not Matched.</li>";
}
...
$("#loginset").submit(validate);/*link function validate() to the submit event
of the form */
